...
package com.ankipro2.anki.provider;
...
import com.ankipro2.anki.FlashCardsContract.CardTemplate;
...
import static com.ankipro2.anki.FlashCardsContract.READ_WRITE_PERMISSION;
    ...
    private int bulkInsertNotes(ContentValues[] valuesArr, long deckId) {
        ...
        try {
            ...
            for (int i = 0; i < valuesArr.length; i++) {
                ...
                // Create empty note
                com.ankipro2.libanki.Note newNote = new com.ankipro2.libanki.Note(col, model); // for some reason we cannot pass modelId in here
                // Set fields
                // Check that correct number of flds specified
    ...
    public Uri insert(Uri uri, ContentValues values) {
        ...
        switch (match) {
            case NOTES: {
                ...
                String tags = values.getAsString(FlashCardsContract.Note.TAGS);
                // Create empty note
                com.ankipro2.libanki.Note newNote = new com.ankipro2.libanki.Note(col, col.getModels().get(modelId));
                // Set fields
                String[] fldsArray = Utils.splitFields(flds);
                // Check that correct number of flds specified
                if (fldsArray.length != newNote.getFields().length) {
